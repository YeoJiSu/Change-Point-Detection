...
def plot_posterior(T, data, cps, R, pmean, pvar, cp_0, cp_1, cp_2, cp_3, cp_20):
    ...
    for cp in cp_0:
        # ax2.axvline(cp, c='red', ls='dotted', lw=2)
        ax2.scatter(cp, 10, color='red', marker='o', s=2, label = 'Predicted CP (r_t = 0)')  # 예측 체인지포인트

    for cp in cp_1:
        ax2.scatter(cp, 20, color='blue', marker='o', s=2, label = 'Predicted CP (r_t = 1)')
        # ax2.axvline(cp, c='yellow', ls='dotted', lw=1.8)
    for cp in cp_2:
        ax2.scatter(cp, 30, color='green', marker='o', s=2, label = 'Predicted CP (r_t = 2)')
        # ax2.axvline(cp, c='green', ls='dotted', lw=1.5)
    for cp in cp_3:
        ax2.scatter(cp, 40, color='purple', marker='o', s=2, label = 'Predicted CP (r_t = 3)')
...
if __name__ == '__main__':
    ...
    var0   = 400      # The prior variance for mean parameter.
    varx   = 200      # The known variance of the data.
    ...
    print("pred_cp_20", cp_20)
    
    # plt.plot(R[1:,0])
    plot_posterior(T, data, cps, R, pmean, pvar, cp, cp_1, cp_2, cp_3, cp_20)
    print("hi")
